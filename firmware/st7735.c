...
static void format_move_str_short(Move move, char *move_str) {
...
static void get_pv_string(const Position *pos, Move best_move, int depth, char *pv_buf, size_t max_len) {
    if (!pos) {
        pv_buf[0] = '\0';
        return;
    }
    ...
    while (pv_move != 0 && pv_ply < 5) {
        char m_str[6];
        format_move_str_short(pv_move, m_str);

        for (int i = 0; i < 5 && m_str[i] != '\0'; i++) {
            m_str[i] = toupper((unsigned char)m_str[i]);
        }

        int added = snprintf(pv_buf + buf_len, max_len - buf_len, "%s%s", (pv_ply > 0) ? " " : "", m_str);
        if (added <= 0 || buf_len + added >= (int)max_len) break;
        buf_len += added;

        if (!make_move(&temp_pos, pv_move)) break;
        pv_ply++;

        pv_move = get_tt_best_move(temp_pos.hash_key);
    }
}
...
void st7735_draw_status(const Position *pos, int level, int side, int current_depth, int score, Move current_best_move, const char *last_move, bool is_thinking, const char *status_msg) {
    ...
    if (score > MATE_VALUE - 100) {
        ...
        int w_score = (side == WHITE) ? score : -score;
    ...
    char line2_buf[24];
    uint16_t line2_color = ST7735_WHITE;

    if (is_thinking) {
        char pv_str[24];
        get_pv_string(pos, current_best_move, current_depth, pv_str, sizeof(pv_str));
        if (strlen(pv_str) > 0) {
            snprintf(line2_buf, sizeof(line2_buf), "%s", pv_str);
        } else {
            snprintf(line2_buf, sizeof(line2_buf), "Thinking...");
        }
        line2_color = ST7735_CYAN;
    } else {
        char move_clean[8] = "";
        if (last_move != NULL && strlen(last_move) > 0) {
            int m_idx = 0;
            for (int i = 0; i < 5 && last_move[i] != '\0'; i++) {
                if (last_move[i] != ' ') {
                    move_clean[m_idx++] = last_move[i];
                }
            }
            move_clean[m_idx] = '\0';
        }

        if (status_msg != NULL && strlen(status_msg) > 0) {
            if (strstr(status_msg, "nAtE") != NULL || strstr(status_msg, "Mate") != NULL || strstr(status_msg, "CHECKMATE") != NULL) {
                if (strlen(move_clean) > 0) {
                    snprintf(line2_buf, sizeof(line2_buf), "%s Mate", move_clean);
                } else {
                    snprintf(line2_buf, sizeof(line2_buf), "Mate");
                }
                line2_color = ST7735_RED;
            } else if (strstr(status_msg, "CHk") != NULL || strstr(status_msg, "CHECK") != NULL || strstr(status_msg, "Check") != NULL) {
                if (strlen(move_clean) > 0) {
                    snprintf(line2_buf, sizeof(line2_buf), "%s Check", move_clean);
                } else {
                    snprintf(line2_buf, sizeof(line2_buf), "Check");
                }
                line2_color = ST7735_RED;
            } else if (strstr(status_msg, "drAU") != NULL || strstr(status_msg, "DRAW") != NULL || strstr(status_msg, "Draw") != NULL) {
                if (strlen(move_clean) > 0) {
                    snprintf(line2_buf, sizeof(line2_buf), "%s Draw", move_clean);
                } else {
                    snprintf(line2_buf, sizeof(line2_buf), "Draw");
                }
                line2_color = ST7735_GRAY;
            } else {
                snprintf(line2_buf, sizeof(line2_buf), "%s", (strlen(move_clean) > 0) ? move_clean : status_msg);
            }
        } else {
            snprintf(line2_buf, sizeof(line2_buf), "%s", (strlen(move_clean) > 0) ? move_clean : "----");
        }
    }
